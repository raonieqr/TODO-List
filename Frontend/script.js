...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function createTaskObj() {
    ...
    if (
      !check.isValidDate(inputValues.dateTime) ||
  ...
  function showTaskList() {
    if (taskObj.length <= 0) alert("Lista vazia");
    else {
  ...
  function handleDeleteTask(index, row) {
    ...
    if (checkedCheckboxes.length === 0) actionButton.style.display = "none";
  }
  ...
  function createTaskRow(task) {
    ...
    let c8 = createTableCell(
      '<i id="pencil" class="ph ph-pencil-line"></i>' +
    ...
    if (radios.length > 0) {
      ...
      while (selectedRowsArray.length !== 0) {
        ...
        taskObj.forEach((task) => {
          if (task.id === index) task.status = valueRadio;
        });
      }
    } else {
  ...
  function editTaskHandler(index) {
    ...
    if (
      !check.validateFields(taskInputs) ||
