...
export function isDateInput(dateString) {
  ...
  const diff = new Date(year, parsedMonth - 1, day, parsedHours, parsedMinutes)
                   .getTime() -
               Date.now();

  if (isInvalidMonth(parsedMonth) ||
      isInvalidTime(parsedHours, parsedMinutes) ||
      isInvalidDay(parsedMonth, day) || isNegativeDiff(diff))
    return 0;

  return 1;
}

export function isInvalidMonth(month) { return month > 12; }
...
export function isInvalidDay(month, day) {
  return (month === 2 && day > 29) ||
         ([ 4, 6, 9, 11 ].includes(month) && day > 30) || day > 31;
}

export function isNegativeDiff(diff) { return diff < 0; }

export function isNotEmpty(value) { return value.trim() !== ''; }
